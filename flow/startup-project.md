...
- 若发现 Dev-Spec 候选文件，先优先用原生选择框询问用户是否加载；1 个候选提供“加载 / 不加载”，2-3 个候选可逐项询问，超过 3 个候选先让用户选择“加载推荐/全选 / 选择部分 / 不加载”，用户未选择前不读取正文
...
- 若仅发现 Dev-Spec 候选，先提示用户选择是否加载；优先使用原生选择框，多选或超过 3 个候选时按“加载推荐/全选 / 选择部分 / 不加载”处理；选定后直接进入分析
- 先完成方案分析
