...
- 若发现 Dev-Spec 候选文件，先优先用原生选择框询问用户是否加载；1 个候选提供“加载 / 不加载”，2-3 个候选可逐项询问，超过 3 个候选先让用户选择“加载推荐 / 全部加载 / 不加载”，若用户需要只加载部分文件，引导其通过 free-form Other 按编号回复，例如 `1,3`；用户未选择前不读取正文
...
- 若仅发现 Dev-Spec 候选，先提示用户选择是否加载；优先使用原生选择框，多选或超过 3 个候选时按“加载推荐 / 全部加载 / 不加载”处理，部分加载通过 free-form Other 输入编号；选定后直接进入分析
- 先完成方案分析
...
3. 若已扫描到 Dev-Spec 候选但未选择，先输出候选列表，并优先用原生选择框等待用户选择是否加载
