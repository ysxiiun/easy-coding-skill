...
先从当前已加载 `$with-claude` skill 的实际入口解析 `<with-claude-skill-dir>`，再调用：

```bash
python3 -B <with-claude-skill-dir>/scripts/run_claude_worker.py --cwd "$PWD" < task-packet-stdin
```

不得猜测 `$HOME`、用户目录或源码仓库中的 fallback。已加载 skill 路径无法解析或 wrapper 不存在时，按 Claude 不可用降级；无论是否成功调用，都必须保持同等只读约束。
...
2. 构建 Claude 只读任务包，包含用户需求、当前项目模式、已读文件摘要、候选 Dev-Spec 状态、冲突点和待决策点；若命中 Canonical Spec，必须携带 `spec_id`、诊断用 `source_digest`、`design_digest`、`design_scope_digest`、`execution_revision`、`execution_scope_digest`、`selected_task_ids`、`repo_ids` 和 `dependency_summary`。
...
- Host 与 Claude packet 必须记录相同 `design_sha256` 和 `design_scope_sha256`；Claude 返回期间若只有 `execution_revision / execution_scope_sha256` 变化，Host 刷新共享进度后继续使用只读分析；若设计或设计范围摘要变化，丢弃 worker 结果并回到 `ANALYSIS`。`source_sha256` 仅作整文诊断，不作为静态方案失效门禁。
...
5. Canonical task 只能修改已确认的 `change_ids` path/symbols，并按 `step_ids` 与 `test_ids` 执行；Host 按共享 writer 顺序更新 task/Step 状态，Claude 不参与 IMPLEMENT。
...
6. Canonical Spec 场景下，review packet 继续携带已确认的 task/change/test ID、诊断用 `source_digest`、`design_digest`、`design_scope_digest`、`execution_revision` 和 `execution_scope_digest`；review 不得借机扩大实现范围。只有 execution 变化时刷新进度，设计摘要变化时丢弃旧 review 并返回 `ANALYSIS`。
...
- Canonical task：确认后先刷新 execution；integration 已满足时由 Host 写 `completed`，未满足时保留 `verified` 并明确 Canonical task 尚未完成
